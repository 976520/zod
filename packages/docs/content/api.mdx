...
Define your own string formats with `z.stringFormat()`.
...
z.stringFormat("typeid", (val) => {
  return val.length > 10; // custom refinements
});
```
...
This is a clean and powerful approach with a lot of advantages. 

1. It uses language-level features ([destructuring syntax](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring)) instead of library-specific APIs
2. It works in Zod and Zod Mini — no method required
